...
function Events({savedEvents}: any) {
        ...
        {savedEvents.map((event: any) => (
          <li datatest-id="eventList" className="relative" key={event.id}>
                <div className="event -mx-3 flex cursor-pointer items-center rounded-md p-3 transition duration-300 ease-in-out hover:bg-slate-100 ">
                  ...
                  <div className="pr-10">
                    <div    className="event__title truncate">{event.title}</div>
                    <div className="mt-0.5 text-xs text-slate-500">
                      <span className="event__days">Starts : {event.start} </span> 
                      <br/>
                      <span className="event__days">Ends : {event.end}</span> 
                    </div>
                  </div>
                </div>
                <div className="flex">
                <button
                type="button" 
                  className="absolute inset-y-0 right-12 text-gray-400 hover:text-gray-700 my-auto flex items-center"
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                ...
                <button
                type="button" 
                  className="absolute inset-y-0 right-0 text-gray-400 hover:text-gray-700 my-auto flex  items-center"
